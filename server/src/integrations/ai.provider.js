...
/**
 * OpenAI ile ilgili hataları uygulama genelinde
 * standart biçimde taşımak için kullanılır.
 */
...
class AIProvider {
  ...
  /**
   * Provider'ın kullanıma hazır olup olmadığını döndürür.
   */
  isAvailable() {
  ...
  /**
   * Düz metin çıktısı üretir.
   *
   * @param {object} params
   * @param {string} params.instructions Sistem talimatı
   * @param {string|Array<object>} params.input Kullanıcı girdisi
   * @param {string} [params.model] Kullanılacak model
   * @param {number} [params.maxOutputTokens] Maksimum çıktı token miktarı
   * @param {object} [params.metadata] OpenAI isteğiyle ilişkilendirilecek metadata
   * @param {string} [params.previousResponseId] Konuşma devamlılığı için response ID
   */
  ...
  /**
   * JSON Schema ile yapılandırılmış çıktı üretir.
   *
   * Modelin rastgele metin veya markdown yerine tanımlanan
   * JSON şemasına uygun sonuç üretmesini amaçlar.
   *
   * @param {object} params
   * @param {string} params.instructions Sistem talimatı
   * @param {string|Array<object>} params.input Kullanıcı girdisi
   * @param {string} params.schemaName Şema adı
   * @param {object} params.schema JSON Schema
   * @param {string} [params.schemaDescription] Şema açıklaması
   * @param {string} [params.model] Kullanılacak model
   * @param {number} [params.maxOutputTokens] Maksimum çıktı token miktarı
   * @param {object} [params.metadata] İstek metadata bilgileri
   */
  async createStructuredResponse({
    ...
    try {
      ...
      const outputText = this.extractOutputText(response);
      const parsedOutput = this.parseStructuredOutput(outputText);
  ...
  /**
   * OpenAI'a gönderilmiş bir dosya ID'siyle analiz yapar.
   *
   * Not:
   * Dosyayı OpenAI'a yükleme işlemini ayrı metotta yapıyoruz.
   * Böylece aynı dosya gerektiğinde tekrar kullanılabilir.
   */
  async createFileResponse({
    ...
    if (!fileId || typeof fileId !== 'string') {
      throw new AIProviderError('Geçerli bir OpenAI fileId zorunludur.', {
    ...
    if (!prompt || typeof prompt !== 'string') {
      throw new AIProviderError('Dosya analizi promptu zorunludur.', {
  ...
  if (
    ...
    throw new AIProviderError(
      'Structured dosya çıktısı için schemaName zorunludur.',
      {
        code: 'INVALID_SCHEMA_NAME',
        statusCode: 500,
      }
  ...
  if (
    ...
    throw new AIProviderError(
      'Structured dosya çıktısı için geçerli JSON Schema zorunludur.',
      {
        code: 'INVALID_JSON_SCHEMA',
        statusCode: 500,
      }
  ...
  try {
    ...
    logger.info('OpenAI structured dosya cevabı oluşturuldu', {
      responseId: result.responseId,
      fileId,
      model: result.model,
      schemaName,
  ...
  /**
   * Buffer içeriğini OpenAI Files API'ye yükler.
   *
   * @param {object} params
  ...
  async uploadFile({
    ...
    try {
      ...
      logger.info('Dosya OpenAI sistemine yüklendi', {
      ...
      return {
        ...
        createdAt: uploadedFile.created_at
          ? new Date(uploadedFile.created_at * 1000).toISOString()
          : null,
      };
    } catch (error) {
      throw this.handleError(error, {
  ...
  /**
   * OpenAI tarafındaki geçici dosyayı siler.
   * Hukuki belgelerin gereksiz yere harici sistemde kalmasını önler.
   */
  ...
  /**
   * Provider bağlantısını düşük maliyetli bir istekle kontrol eder.
   * Bunu her normal request'te çağırmamalıyız.
   */
  async healthCheck() {
    if (!this.isAvailable()) {
      return {
    ...
    try {
      ...
      return {
        healthy: this.extractOutputText(response)
          ...
          .includes('OK'),
        enabled: true,
        model: response.model || config.OPENAI_MODEL,
        responseId: response.id || null,
        durationMs: Date.now() - startedAt,
      };
    } catch (error) {
      logger.error('OpenAI health check başarısız', {
      ...
      return {
        ...
        model: config.OPENAI_MODEL,
        durationMs: Date.now() - startedAt,
        message: 'OpenAI bağlantı kontrolü başarısız.',
      };
    }
  }
  ...
  ensureAvailable() {
    ...
    if (!this.client || !config.OPENAI_API_KEY) {
      throw new AIProviderError(
  ...
  validateCommonInput({
    ...
    if (
      ...
      throw new AIProviderError('OpenAI input alanı zorunludur.', {
    ...
    if (!model || typeof model !== 'string') {
      throw new AIProviderError('Geçerli bir OpenAI modeli zorunludur.', {
  ...
  parseStructuredOutput(outputText) {
    try {
      ...
      logger.error('Structured AI cevabı JSON olarak ayrıştırılamadı', {
  ...
  normalizeUsage(usage) {
    ...
    return {
      ...
      inputDetails: usage.input_tokens_details || null,
  ...
  handleError(error, context = {}) {
    ...
    logger.error('OpenAI provider hatası', {
